module SlackGamebot
    module Commands
      class Tied < SlackRubyBot::Commands::Base
        def self.call(client, data, match)
          ...
          with_teams = expression && (expression.include?(' vs ') || expression.include?(' vs. '))
          ...
          if opponent.nil? || scores.nil? || scores.empty?
            client.say(channel: data.channel, text: "Please enter the scores in the form `pp tied @opponent1 @opponent2 with @teammate 41:55 using Your Team vs. Their Team`", gif: 'error')
            return
          end
